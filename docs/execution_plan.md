...
## Paper draft
Status: **compile-ready draft exists** — `paper/main.tex`, all 15 required
sections, compiles clean (0 errors, no undefined refs/citations) via
`bash scripts/compile_paper.sh` on a user-space TeX Live
(`docs/reproducibility_texlive.md`) verified during this project. Every
table is `\input` from `scripts/csv_to_latex_tables.py`'s output (never
hand-typed); every figure is generated by `scripts/make_figures.py` from
the same committed CSVs. Bibliography: 16 `VERIFIED` sources only. Every
results section is explicitly marked synthetic-tier / pipeline-validation,
with `\RESULTPENDING{}` on the real-data claims and `\TODO{}` on the
not-yet-implemented coarse-ontology sensitivity analysis. This is a status
report on an incomplete study, not a polished conclusion — read
§"Discussion"/"Conclusion" in the draft itself for the exact framing.
