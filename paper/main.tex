...
paper applies it per-client via Opacus. \textbf{Gap:} neither mechanism, by
itself or combined (as we test directly, \S\ref{sec:privacy}), is a
capability-isolation mechanism -- both protect \emph{how training happened},
not \emph{what the resulting model exposes to different recipients}, and
this paper does not claim secure aggregation prevents the membership or
property inference it separately evaluates in \S\ref{sec:security}.

\paragraph{Inference and extraction attacks.} Deep Leakage from Gradients
\citep{zhu2019deep} reconstructs a training input from its raw gradient;
membership inference \citep{shokri2017membership} distinguishes training
members from non-members via model behavior; model extraction
\citep{tramer2016stealing} clones a model's functionality from black-box
query access. All three are implemented in \S\ref{sec:security} against
MedGate-FL specifically, adapted to ask whether the \emph{public} path
leaks the \emph{restricted} task -- a different question from the attacks'
original centralized or generically-federated framing. \UNVERIFIED{iDLG;
Geiping et al.\ inverting-gradients; Nasr et al.\ comprehensive privacy
analysis; Melis et al.\ property inference; Fredrikson et al.\ model
inversion; Hitaj et al.\ GAN-based leakage -- not cited pending
verification; several of these (Melis et al.\ in particular) would be the
natural source for a property-inference attack this paper has not yet
implemented, see \S\ref{sec:limitations}.}

\paragraph{Federated and medical unlearning.} \citet{chen2026lethe} (Lethe)
benchmark twelve federated-unlearning methods across eight medical-imaging
tasks and multiple removal granularities, finding that the
\emph{difficulty of the forgetting request}, not the unlearning method,
dominates performance differences, and that client-level removal often has
minimal utility impact -- making residual membership signal, not utility
loss, the metric that actually distinguishes methods. This is the closest
prior benchmark to this paper's Phase 5 (\S\ref{sec:unlearning}), and its
finding directly motivates measuring forgetting via a membership-inference
proxy rather than utility drop alone, which is what we do.
\UNVERIFIED{Wu et al.\ federated unlearning via knowledge distillation;
Halimi et al.\ efficient client erasure; Zhang et al.\ FedRecovery; Deng
et al.\ right-to-be-forgotten medical unlearning -- not cited pending
verification; this paper's \texttt{gradient\_ascent\_unlearning} method
(\S\ref{sec:unlearning}) is therefore described only as a generic simple
technique, not attributed to any of these.} \textbf{Gap:} no cited,
verified prior work evaluates unlearning specifically against a
capability-isolated architecture, where an ``adapter deletion'' operation
is available that has no counterpart in a monolithic model.

\paragraph{Access control and cryptography, and blockchain-assisted FL.}
\UNVERIFIED{Every specific source the project brief names in these two
categories (CP-ABE, ABE, probabilistic encryption, FHE constructions,
blockchain-FL systematic reviews) remains unverified as of this draft; see
\texttt{docs/literature\_matrix.md}.} This paper's crypto layer
(\S\ref{sec:privacy}) is accordingly built from first principles against
current, maintained libraries (AES-GCM via Python's \texttt{cryptography}
package) rather than citing a specific ABE or FHE construction it does not
implement. Blockchain-assisted authorization (project brief Phase 8) is not
yet implemented; see \S\ref{sec:limitations}.
